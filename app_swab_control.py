...
LABELS_COLUMNAS = {
    ...
    "PRAG_MES_ANTERIOR_OBJETIVO": "Producción de agua mes anterior al objetivo",
...
def asegurar_columnas_unicas(columnas):
    """
    Evita errores de Streamlit/Arrow cuando dos columnas terminan con el mismo nombre visible.
    """
    conteo = {}
    nuevas = []
    ...
    for col in columnas:
        ...
        if nombre not in conteo:
            ...
            conteo[nombre] += 1
            nuevas.append(f"{nombre} ({conteo[nombre] + 1})")
...
def formatear_tabla(df):
    """
    Prepara tablas para Streamlit, Excel y PPT.
    Corrige columnas mixtas para evitar el error de conversión a Arrow.
    """
    salida = df.copy()
    ...
    # Algunas columnas llegan como object con mezcla de fechas, números y textos.
    # Streamlit usa Arrow y puede fallar si una columna tiene tipos mezclados.
    for col in salida.select_dtypes(include=["object"]).columns:
        salida[col] = salida[col].where(pd.notna(salida[col]), "")
        salida[col] = salida[col].astype(str)

    salida = salida.rename(columns=LABELS_COLUMNAS)
    salida.columns = asegurar_columnas_unicas(salida.columns)
...
def formatear_tabla_streamlit(df):
    """
    Versión ultra segura para mostrar tablas en Streamlit.
    Convierte todo a texto para evitar errores Arrow cuando hay columnas mixtas.
    No afecta los cálculos ni las descargas, solo la tabla visual.
    """
    salida = formatear_tabla(df).copy()
...
def obtener_mes_objetivo_default(df):
    """
    Usa por defecto el último mes completo.
    Si la data llega, por ejemplo, hasta 05/06/2026, el mes objetivo
    por defecto será mayo 2026, no junio 2026.
    """
    fecha_maxima = df["FECHA"].max()
    ultimo_dia = ultimo_dia_mes(fecha_maxima.year, fecha_maxima.month)

    if fecha_maxima.normalize() < ultimo_dia.normalize():
        fecha_default = pd.Timestamp(fecha_maxima.year, fecha_maxima.month, 1) - pd.DateOffset(months=1)

        existe_mes_default = (
            (df["ANIO"] == fecha_default.year) &
            (df["MES"] == fecha_default.month)
        ).any()

        if existe_mes_default:
            return int(fecha_default.year), int(fecha_default.month)

    return int(fecha_maxima.year), int(fecha_maxima.month)


def mes_objetivo_incompleto(df, anio, mes):
    data_mes = df[(df["ANIO"] == int(anio)) & (df["MES"] == int(mes))].copy()
    if data_mes.empty:
        return False, None, None

    fecha_max_mes = data_mes["FECHA"].max()
    fecha_fin_mes = ultimo_dia_mes(anio, mes)

    return fecha_max_mes.normalize() < fecha_fin_mes.normalize(), fecha_max_mes, fecha_fin_mes
...
anio_default, mes_default = obtener_mes_objetivo_default(df)
...
with st.sidebar:
    ...
    incompleto, fecha_max_mes_obj, fecha_fin_mes_obj = mes_objetivo_incompleto(df, anio_objetivo, mes_objetivo)
    if incompleto:
        st.warning(
            f"Ojo: {MESES[int(mes_objetivo)]} {int(anio_objetivo)} está incompleto. "
            f"La data llega hasta {fecha_max_mes_obj.date()} y el mes cierra el {fecha_fin_mes_obj.date()}. "
            "Para un análisis de pozos dejados más estable, usa el último mes completo."
        )
...
with tabs[0]:
    ...
    st.dataframe(formatear_tabla_streamlit(tabla_desp[cols_desp]), use_container_width=True, hide_index=True)

    # Gráficas enfocadas SOLO en pozos dejados de hacer.
    # No se incluyen pozos reducidos porque la presentación debe centrarse en los pozos que no se hicieron.
    ...
    if data_graf_pozos.empty:
        ...
        # Segunda gráfica usando los mismos pozos y el mismo orden de la gráfica superior.
        # Así se ve primero el petróleo base no realizado y debajo el potencial estimado.
        orden_pozos_dejados = data_graf_pozos["POZO"].tolist()
    ...
    with col_g3:
        bubble = desplazamiento[desplazamiento["ESTADO_DESPLAZAMIENTO"] == "DEJADO DE HACER"].copy()
        if not bubble.empty:
            fig_bubble = px.scatter(
                ...
                hover_name="POZO",
                hover_data=[
                    "BATERIA", "CLASIFICACION", "INTERV_BASE",
    ...
    data_graficos = {
        ...
        "Graf bateria dejados": resumen_desplazamiento_por_bateria(
            desplazamiento[desplazamiento["ESTADO_DESPLAZAMIENTO"] == "DEJADO DE HACER"].copy()
        )
    }
...
with tabs[1]:
    ...
    if not afectados.empty:
        ...
        st.dataframe(formatear_tabla_streamlit(resumen_afectados), use_container_width=True, hide_index=True)
...
with tabs[2]:
    ...
    if listado_convertidos.empty:
        ...
        st.dataframe(formatear_tabla_streamlit(resumen_conv), use_container_width=True, hide_index=True)

        st.write("Listado de pozos convertidos")
        st.dataframe(formatear_tabla_streamlit(tabla_conv), use_container_width=True, hide_index=True)
...
with tabs[4]:
    ...
    st.dataframe(formatear_tabla_streamlit(tabla_pozos[cols_pozos].sort_values(["ESTADO", "CLASIFICACION", "BATERIA", "POZO"])), use_container_width=True, hide_index=True)
...
with tabs[5]:
    ...
    if res_tipo.empty:
        ...
        st.dataframe(formatear_tabla_streamlit(res_tipo), use_container_width=True, hide_index=True)
...
with tabs[6]:
    ...
    st.dataframe(formatear_tabla_streamlit(validacion), use_container_width=True, hide_index=True)
